...
class $modify(MyGlobalReplaces, MenuLayer) {
    ...
    void performGlobalReplaces(float dt) {
        ...
        if (originalFrame && geodeFrame) {
            ...
            log::info("Salamandra: GJ_featuredBtn_001.png has been successfully hacked globally.");
        } else {

            log::error("Salamandra: The original frame or the Geode frame for global replacement could not be found.");
        }
    }
};